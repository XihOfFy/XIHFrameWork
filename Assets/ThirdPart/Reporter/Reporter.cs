...
public class Reporter : MonoBehaviour
{
    ...
    bool isGestureDone()
    {
        ...
        {
            var t0 = UnityEngine.InputSystem.EnhancedTouch.Touch.activeTouches;
            if (t0.Count != 1)
            {
            ...
            {
                if (t0[0].phase == TouchPhase.Canceled || t0[0].phase == TouchPhase.Ended)
                    gestureDetector.Clear();
                else if (t0[0].phase == TouchPhase.Moved)
                {
                    Vector2 p = t0[0].screenPosition;
    ...
    bool isDoubleClickDone()
    {
           ...
           Application.platform == RuntimePlatform.IPhonePlayer)
        {
            ...
            {
                if (t0[0].phase == TouchPhase.Began)
                {
                    if (lastClickTime == -1)
    ...
    Vector2 getDownPos()
    {
        ...
        {
            var t0 = UnityEngine.InputSystem.EnhancedTouch.Touch.activeTouches;
            if (t0.Count == 1 && t0[0].phase == TouchPhase.Began)
            {
                downPos = t0[0].screenPosition;
                return downPos;
            }
        }
        else
    ...
    Vector2 getDrag()
    {
            ...
            Application.platform == RuntimePlatform.IPhonePlayer)
        {
            ...
            //return Mouse.current.position.ReadValue() - downPos;
        }
        else
        {
            //if (Input.GetMouseButton(0)) {
    ...
    void Update()
    {
        ...
        if (sceneIndex >= 0 && scenes.Length > sceneIndex && string.IsNullOrEmpty(scenes[sceneIndex]))
            scenes[sceneIndex] = SceneManager.GetActiveScene().name;
#else
