...
namespace TTSDK
{
    public class TTFileSystemManagerDefault : TTFileSystemManager
    {
        ...
        {
            var err = WriteInternal(param.fd, param.data, param.offset, param.length ?? (param.data.Length - param.offset), param.position);
            if (err == null)
            {
        ...
        private string WriteInternal(string fd, byte[] data, int offset, int length, int? position)
        {
            if (!_openFiles.TryGetValue(fd, out var fs))
                return "bad file descriptor";
            try
